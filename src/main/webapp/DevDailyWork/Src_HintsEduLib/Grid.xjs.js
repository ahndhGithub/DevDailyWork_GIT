...
(function()
{
    return function(path)
    {
        ...
        // User Script
        this.addIncludeScript(path,"Src_HintsEduLib::All_EduLib.xjs");
        ...
        */
        pForm._gfnGridPasteEvent = function (obj, e)
        {
        	var gtrcPos = "Grid.xjs._gfnGridPasteEvent";
        		this.gtrace("● 붙여넣기(ctrl+V)", gtrcPos);

        	var browser = system.navigatorname;	this.gtrace("browser--->"+browser, gtrcPos);
        	var copyData = this._gfnGirdGetPasteData(browser);	this.gtrace("copyData--->"+copyData, gtrcPos);
        	...
        	if( browser == "nexacro" || browser =="IE"){
        		...
        		this.gtrace("rowData--->"+rowData, gtrcPos);

        	var rowDataCount = rowData.length==1?1:rowData.length - 1;	this.gtrace("rowDataCount--->"+rowDataCount, gtrcPos);
        	...
        	var datasetName = obj.binddataset;		this.gtrace("datasetName--->"+datasetName, gtrcPos);
        	var ds = obj.getBindDataset();			this.gtrace("ds--->"+ds, gtrcPos);

        	ds.set_enableevent(false);

        	var grdCellCount = obj.getCellCount("body");		this.gtrace("grdCellCount--->"+grdCellCount, gtrcPos);
        	var rowCount = ds.getRowCount();					this.gtrace("rowCount--->"+rowCount, gtrcPos);

        	var startrow = nexacro.toNumber(obj.selectstartrow);	this.gtrace("startrow--->"+startrow, gtrcPos);
        	if( startrow == -9) return;
        	...
        	var startcol = 0;
        	var endcol = 0;
        														this.gtrace("obj.selecttype--->"+obj.selecttype, gtrcPos);
        														this.gtrace("obj.getCurEditType()--->"+obj.getCurEditType(), gtrcPos);
        	if(obj.getCurEditType() == "none")
        	{
        		if( obj.selecttype == "row" || obj.selecttype == "multirow"){
        			startcol = 0;
        			endcol = obj.getCellCount("body")-1;
        			...
        			endcol   = nexacro.toNumber(obj.selectendcol);
        														...
        														this.gtrace("obj.getCellPos()--->"+obj.getCellPos(), gtrcPos);
        		if( obj.selecttype == "row" || obj.selecttype == "multirow"){
        			startcol = obj.getCellPos();
        			endcol = obj.getCellCount("body")-1;
        		}else{
        			startcol = nexacro.toNumber(obj.getCellPos());
        			endcol   = nexacro.toNumber(obj.selectendcol);
        		}
        	}
        														this.gtrace("startcol == cellIndex --->"+startcol, gtrcPos);
        														this.gtrace("endcol--->"+endcol, gtrcPos);
        	var currRow = startrow;
        	var cellIndex = startcol;
        	var maxColumnCount = 0;
        	var checkIndex = {};
        												this.gtrace("ds.name-->"+ds.name,gtrcPos);
        	var iRowStsColIdx = ds.getColIndex( "ROW_STS" ); 	this.gtrace("iRowStsColIdx-->"+iRowStsColIdx,gtrcPos);

        														this.gtrace("rowDataCount--->"+rowDataCount, gtrcPos);
        	for (var i = 0; i < rowDataCount; i++)
        	{
        		...
        		if(maxColumnCount < columnLoopCount) {
        			maxColumnCount = columnLoopCount;
        												...
        												this.gtrace("cellIndex-->"+cellIndex,gtrcPos);
        												this.gtrace("columnLoopCount-->"+columnLoopCount,gtrcPos);
        		var k = 0;
        ...
        // 			}
        			...
        			colid = this.gfn_getBindColId(obj, j);

        			var tempValue = columnData[k];
        			if(!this.gfn_isNull(tempValue))
        			{
        												this.gtrace("**********currRow**********---->"+currRow, gtrcPos);
        												this.gtrace("colid.dsCol---->"+colid.dsCol, gtrcPos);
        												this.gtrace("tempValue---->"+tempValue, gtrcPos);
        				ds.setColumn(currRow, colid.dsCol, tempValue);
        				if(ds.getColumn(currRow, "ROW_STS")!="I"){
        					ds.setColumn(currRow, "ROW_STS", "U");
        				}
        			}
        			k++;
        		}
        	...
        	//	system.clearClipboard(); //클리어되지 않도록 함. 2020-03-04
        ...
        */
        pForm.gfn_setGrdDic = function(objGrid){
        	...
        	// 바인드명을헤드텍스트로 (ds_grdDic)
        	var sTrcGridNm = "GridDevTableCreate";
        	var sTrcTmpCol = "DATA_TYPE";
