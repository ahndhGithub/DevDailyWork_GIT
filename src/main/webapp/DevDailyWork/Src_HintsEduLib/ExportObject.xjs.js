    ...
    return function(path)
    {
        ...
        if (!nexacro.ExcelExportObject) {
        	...
        	_pExcelExport.exportData = function (argsParam, argsDsParam, nOrgValueType, bFileDownload) {
        								trace("here");
        							var gtrcPos = "ExportObject.xjs.exportData";
        												this.gtrace("● 익스포트데이터",gtrcPos);
        		this._argsParam = argsParam;				this.gtrace(" - 아규먼트 (this._argsParam)--->"+this._argsParam,gtrcPos);
        	...
        	_pExportItem._exportCallback = function (svcid, errstatus, message) {
        											var gtrcPos = "ExportObject.xjs._exportCallback";
        											this.gtrace("<---엑셀내보내기콜백--->"+arrProp, gtrcPos);
        		var grid = this.source;
        		var exportObj = this.parent;
        		var exportbar = exportObj._exportBar;
        		if (errstatus < 0) {
        			...
        			if (is_end) {
        				...
        				this._gridExportContinue(exportObj);
        											...
        											this.gtrace("<---엑셀내보내기콜백--------끝--->"+arrProp, gtrcPos);
        	};
        	...
        	_pExportItem._gridItemExport = function (exportObj) {
        																var gtrcPos = "ExportObject.xjs._gridItemExport";
        		...
        		this._rollbackSuppressInfo();
        															this.gtrace("export_dsparam--->"+export_dsparam, gtrcPos);
        															this.gtrace("this.id--->"+this.id, gtrcPos);
        															this.gtrace("exportObj._exporturl--->"+exportObj._exporturl, gtrcPos);
        															this.gtrace("COMMAND=_ds_command STYLE=_ds_style CELL=_ds_cell" + " " + export_dsparam, gtrcPos);
        // 															this.gtrace("export_param--->"+export_param, gtrcPos);
        // 															this.gtrace("export_dsparam--->"+export_dsparam, gtrcPos);
