...
(function()
{
    return function(path)
    {
        ...
        *****************************************************************************************************/
        	 ...
        	 {	//	out데이터셋백업 : 콜백 사라짐 제거용
        		...
        		for (var k = 0 ; k < arrOutDs.length ; k++)
        		{
        			...
        			//	var tmpUseclientlayout = tmpDs.useclientlayout;
        					...
        					this.gtrace("tmpDs.saveXML()---->"+tmpDs.saveXML(), gtrcPos);
        ...
        /**
        * @class	Result데이터셋을 추가
        * @param
        * @return
        * @example
        *
        */
        pForm.gfn_addResultDs = function(oDs)
        {
        	var resultDsName = oDs.name+"_Result";

        	if(!this.lookup(resultDsName)){
        		var tResultDs = new Dataset;
        		this.addChild(resultDsName, tResultDs);
        	}
        ...
        /**
         * @함수설명  				트랜잭션 전 공통처리 (데이터셋, ...) + 트랜잭션전용 데이터셋 없다면 추가
         * @param arrCode
         * @param isAsync
         * @return None
         */
        this.gfn_beforeTransDsAdd = function(orgDs, StsChkType)
        {
        	var transDsName;
        		transDsName = orgDs.name+"_Tran";

        	var tTransDs;
        	if(!this.lookup(transDsName)){
        		tTransDs = new Dataset;
        		this.addChild(transDsName, tTransDs);
        	}else{
        		tTransDs = this.lookup(transDsName);
        	}

        	this.gfn_beforeTrans(orgDs, tTransDs, StsChkType);
        }
        });
