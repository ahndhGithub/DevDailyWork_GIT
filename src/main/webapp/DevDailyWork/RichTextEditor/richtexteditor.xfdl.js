(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj.getSetter("scrollbars").set("none");
            obj.set_visible("false");
            obj.set_url("RichTextEditor::richtexteditor_colorpicker.xfdl");
            this.addChild(obj.name, obj);
        ...
        {
            this._addPreloadList("fdl","RichTextEditor::richtexteditor_colorpicker.xfdl");
        };
        
        // User Script
        this.addIncludeScript("richtexteditor.xfdl","Library_RichTextEditor::lib_base.xjs");
        this.addIncludeScript("richtexteditor.xfdl","Library_RichTextEditor::lib_array.xjs");
        this.addIncludeScript("richtexteditor.xfdl","Library_RichTextEditor::lib_color.xjs");
        ...
        this.executeIncludeScript("Library_RichTextEditor::lib_array.xjs"); /*include "Library_RichTextEditor::lib_array.xjs"*/;
        this.executeIncludeScript("Library_RichTextEditor::lib_color.xjs"); /*include "Library_RichTextEditor::lib_color.xjs"*/;
        ...
        // FTP download URL
        var FTP_URL = domain + "FTPImageDownload.do?UUID=";

        // 편집 기본 설정
        var initConfig = {
        	editor : {
        		// editor.html 의 경로
        		url : domain + "RichTextEditor/html/editor.html",
        		//url : "http://localhost:8080/DevDailyWork/RichTextEditor/html/editor.html",
        	...
        	image : {
        		// 이미지 파일을 등록할 host 경로
        		host : domain + "RichTextEditor/",
        		//host : "http://localhost:8080/DevDailyWork/RichTextEditor/",

        		// 이미지 파일 업로드 url 로 host 를 제외한 경로 지정.
        		uploadUrl : "jsp/saveFile.jsp?path=repository",

        		// 이미지 파일 삭제 url 로 host 를 제외한 경로 지정.
        		deleteUrl : "jsp/deleteFile.jsp?path=repository",
        ...
        ******************************************************************************/
        ...
        this.updateThumbNailPositionSize = function()
        {
        	...
        	if ( this.div_thumbnail.visible )
        	{
        		...
        		this.st_editor.set_bottom(0);
        		this.txt_htmlEditor.set_bottom(0);
        		this.web_editor.set_bottom(0);
        	}
        ...
        ******************************************************************************/
        ...
        this.fileUpTrans_onsuccess = function(obj,e)
        {
        	...
        	// 파일명 업데이트(UUID)
        	this.ds_images.setColumn(0, 'filename', e.datasets[0].getColumn(0, 'filename'))
        ...
        ******************************************************************************/
        ...
        this.addImageThumbnail = function(fileName, saveFileName, fileSize)
        {
            ...
            imgSrc = "URL('" + FTP_URL + fileName + "')";

        	trace("thumbnail_imgSrc : " + imgSrc);
        	...
        	imgDiv._src = FTP_URL + saveFileName;
        ...
        this.removeItemOnClickHandler = function(obj,  e)
        {
        	...
        	trace('thumbnail delete url : ' + url);
        ...
        ******************************************************************************/
        ...
        // 내용에 변화가 있을시 컨텐츠 체크해서 이미지클립보드 붙여넣기일 경우 제거
        this.chkContent = function()
        {
        	var vContent = this.getHTMLValue("editor.content").toString();
        	var vStartIndex = vContent.indexOf('<img src=');
        	var vEndIndex;
        	vEndIndex = vStartIndex == -1 ? -1 : vContent.indexOf('>', vStartIndex);

        	if(vContent.indexOf('<img src=') != -1) {
        		this.setContent(vContent.substring(0, vStartIndex) + vContent.substring(vEndIndex+1, vContent.length));
        		alert('클립보드 이미지는 붙여넣을 수 없습니다');
        	}
        }
        ...
        // 첨부파일 정보를 초기화한다
        this.resetAttachImages = function(attachInfo)
        {
        	for(var i=0; i<this.ds_images.rowcount; i++) {
        		this.removeImageThumbnail(i);
        	}
        ...
        */
        ...
        this.on_initEvent = function()
        {
            ...
            this.addEventHandler("onkeyup",this.RichTextEditor_onkeyup,this);
